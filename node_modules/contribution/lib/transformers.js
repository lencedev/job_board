export const parseContributions = (body) => {
    const bodyMatches = body.matchAll(/data-date="(\d{4}-\d{2}-\d{2}).*>(\d+|No) contribution/g);
    const contributions = {};
    for (const [match, date, contribution] of bodyMatches) {
        contributions[date] = contribution === 'No' ? 0 : parseInt(contribution);
    }
    const sortedContributionData = Object.fromEntries(Object.entries(contributions).sort(([a], [b]) => +new Date(a) - +new Date(b)));
    return sortedContributionData;
};
export const parseGitHubStats = (parsedContributions) => {
    const stats = {
